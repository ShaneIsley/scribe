...
use rusqlite::{params, Connection, OptionalExtension, Transaction as RusqliteTransaction};
use std::collections::{BTreeMap, HashMap};
use std::path::Path;

use super::backend::{DatabaseBackend, Transaction};
...
impl<'conn> Transaction for SqliteTransaction<'conn> {
    ...
    fn insert_item(
        ...
        self.tx.execute(&sql, params.as_slice())?;
        Ok(())
    }
...
impl DatabaseBackend for SqliteBackend {
    ...
    fn load_current_state(&self) -> Result<HashMap<String, String>> {
        let mut stmt = self.conn.prepare("SELECT _item_pk, * FROM items")?;
...
/// Database manager for Scribe (legacy interface for backward compatibility)
pub struct Database {
    conn: Connection,
}
...
impl Database {
    ...
    /// Begin a transaction
    pub fn begin_transaction(&mut self) -> Result<RusqliteTransaction<'_>> {
